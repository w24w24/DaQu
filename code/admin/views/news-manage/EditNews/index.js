 ...
 * @LastEditTime: 2023-05-07 22:12:20
...
// 获取 id
let update = new URL(location.href).searchParams.get("id")
...
editNewsForm.onsubmit = async function(evt) {
  ...
  await fetch(`http://localhost:3000/news/${ update }`,{
    headers:{
...
async function render() {
  let { title,category,content:mycontent,cover:mycover } = await fetch(`http://localhost:3000/news/${ update }`).then(res=>res.json())
  // console.log(obj);

  document.querySelector("#title").value = title
  document.querySelector("#category").value = category,
  // 设置 content：假设用户不进行更改，需要保留原数据
